...
if (!isset($_SESSION['user']) || $_SESSION['user']['role'] !== 'fdo') {
...
try {
    // Verify FDO has access to this appointment (appointment exists; FDO manages appointments)
    $stmt = $pdo->prepare("SELECT id, user_id, patient_id FROM appointments WHERE id = ?");
